...
def get_planner_prompt(user_query: str) -> str:
    """Generate a persona prompt for the researcher based on the query."""
    return f"""Based on the user query: "{user_query}"
...
def get_search_queries_prompt(user_query: str) -> str:
    """Generate prompt for creating 4 distinct search queries."""
    current_date = datetime.now(timezone.utc).strftime('%B %d, %Y')
    
    return f"""Generate exactly 4 distinct Google search queries to research the following question: "{user_query}"
...
def get_publisher_prompt(user_query: str, search_results: list[str]) -> str:
    """Generate prompt for synthesizing search results into a markdown report."""
    results_text = "\n\n---\n\n".join(search_results)
    
    return f"""Based on the following search results, create a comprehensive markdown report answering: "{user_query}"
